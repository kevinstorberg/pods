...
const { getRoleFullname, getRoleAbbreviation } = require('./role-mappings.js');
...
async function launchRole(args) {
    ...
    try {
        const configPath = path.join(SCRIPT_DIR, 'config', 'project.jsonc');
...
async function launchAIAssistant(aiEditor, roleCommand) {
    ...
    try {
        ...
        console.error('You can also edit config/assistants.jsonc to use a different AI assistant.');
        process.exit(1);
    }
